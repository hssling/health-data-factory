...
def test_run_continuous_ingestion_auto_publishes_tb(
    monkeypatch: Any, tmp_path: Path
) -> None:
    registry_path = tmp_path / "registry.yaml"
    manifest_root = tmp_path / "manifests"
...
""".strip(),
    ...
    monkeypatch.setenv("HDB_REGISTRY_PATH", str(registry_path))
    monkeypatch.setenv("HDB_MANIFEST_DIR", str(manifest_root))
    monkeypatch.setenv("HDB_AUTO_PUBLISH_TB", "true")
    ...
    monkeypatch.setattr("pipelines.engine.run_dataset_build", fake_run)
    monkeypatch.setattr("hdb.publish.publish_to_huggingface", lambda dataset_id: {"ok": dataset_id})
    monkeypatch.setattr("hdb.publish.publish_to_kaggle", lambda dataset_id: {"ok": dataset_id})

    result = run_continuous_ingestion()
    assert result["executed_count"] == 1
    assert result["results"][0]["publish"] == "hf:yes,kaggle:yes"
